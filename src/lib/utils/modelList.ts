import {
	...
	LLMFlags,
	LLMTokenizer,
} from "../../types/risu/llm/model.types";
...
for (const [i, model] of LLMModels.entries()) {
	if (model.provider === LLMProvider.OpenAI && model.format === LLMFormat.OpenAICompatible) {
		LLMModels.push({
			...model,
	...
	if (model.provider === LLMProvider.GoogleCloud) {
		LLMModels.push({
			...model,
			id: `${model.id}-vertex`,
 ...
 * AS-IS의 DB STATE를 통해 가져오던 정보를 파라미터로 변경 ( db.enableCustomFlags, db.customFlags, db.customModels )
 * @param id
 * @param enableCustomFlags
 * @param customflags
 * @returns
 */
